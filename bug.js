...
var BugDispatch = {
    ...
    kill: function (n) {
        for (var i = 0; i < this.bugs.length && n; i++) {
            if (this.spawnDelay[i]) clearTimeout(this.spawnDelay[i]);
            n -= this.bugs[i].alive;
            this.bugs[i].die();
...
var MothController = function () {
    var mothOptions = {
        imageSprite: 'moth-sprite.png',
        bugWidth: 200,
        bugHeight: 132,
        num_frames: 2,
        canFly: false,
...
MothController.prototype = BugDispatch;
...
var Bug = {
    ...
    initialize: function (transform, options) {
        ...
        this.element = "bugs";
        this.home = window.document.getElementById(this.element);
    ...
    setPos: function (top, left) {
        this.bug.top = top || this.random(this.options.edge_resistance, this.home.clientHeight - this.options.edge_resistance);

        this.bug.left = left || this.random(this.options.edge_resistance, this.home.clientWidth - this.options.edge_resistance);
    ...
    moveBug: function (x, y, deg) {
        ...
        // transform:
        var trans = "translate(" + parseInt(x + this.home.offsetLeft) + "px," + parseInt(y + this.home.offsetTop) + "px)";
        if (deg) {
    ...
    drawBug: function (top, left) {
        ...
        if (!this.inserted) {
            this.inserted = true;
            this.home.appendChild(this.bug);
        }
    },
    ...
    flyRand: function () {
        this.stop();
        var landingPosition = {};
        landingPosition.top = this.random(this.options.edge_resistance, this.home.clientHeight - this.options.edge_resistance);
        landingPosition.left = this.random(this.options.edge_resistance, this.home.clientWidth - this.options.edge_resistance);
    ...
    flyIn: function () {
        ...
        var side = Math.round(Math.random() * 4 - 0.5),
            ...
            e = d.documentElement,
            g = this.home,
            windowX = g.clientWidth || window.innerWidth || e.clientWidth,
            windowY = g.clientHeight || window.innerHeight || e.clientHeight;
        if (side > 3) side = 3;
        if (side < 0) side = 0;
        var style = {},
            s;
    ...
    drop: function (deathType) {
        var startPos = this.bug.top,
            ...
            e = d.documentElement,
            g = this.home,
            finalPos = g.clientHeight || window.innerHeight || e.clientHeight,
    ...
    next_stationary: function () {
        this.toggle_stationary_counter = this.random(20, 50);
    },
    ...
    bug_near_window_edge: function () {
        ...
        else if (this.bug.top > this.home.clientHeight - this.options.edge_resistance)
            this.near_edge |= this.NEAR_BOTTOM_EDGE;
        if (this.bug.left < this.options.edge_resistance)
            this.near_edge |= this.NEAR_LEFT_EDGE;
        else if (this.bug.left > this.home.clientWidth - this.options.edge_resistance)
